...
export default function ReturnRate() {
  ...
  const allVisualsMap: Record<string, VisualConfig[]> = {
    'ReportSectiona37d01e834c17d07bbeb': [
      { visualName: '70748de9ae05c6029e25', stateSetter: setReturns },
      { visualName: 'd4d72ae2a051be63470a', stateSetter: setUnitsReturned },
    ],
  };
...
const handleVisualSelection = async (sourceVisualKey: string, selection: any) => {
  ...
  let filter: powerbi.models.IBasicFilter | null = null;
  let targetVisualKey: string;

  if (sourceVisualKey === 'store') {
    // Define the array of possible store values to randomize from
    const storeOptions = ['Barba', 'Contoso', 'Natura', 'Pomum', 'Fama'];
    
    // Randomly select a store from the options array
    const randomStore = storeOptions[Math.floor(Math.random() * storeOptions.length)];
    
    // Update the selected category with the randomly selected store
    setSelectedCategory(randomStore);
    console.log(`Set Store filter to "${randomStore}" (randomly selected) due to selection in storeRef visual`);
    ...
    targetVisualKey = 'category'; // Apply to the other visual
  } else {
    // Handle selections from other visuals
    ...
    if (!selectedValue) {
      console.warn('No selected value found in identity.equals');
      return;
    }

    // Apply replacement logic
    if (selectedValue === "Pirum" || selectedValue === "Kaizala" || selectedValue === "Exchange") {
      selectedValue = "Leo";
      console.log(`Replaced "${identity.equals}" with "Leo"`);
    } else if (selectedValue === "OneNote" || selectedValue === "Planner") {
      selectedValue = "Fama";
      console.log(`Replaced "${identity.equals}" with "Fama"`);
    } else if (selectedValue === "Publisher") {
      selectedValue = "Abbas";
      console.log(`Replaced "Publisher" with "Abbas"`);
    } else if (selectedValue === "SharePoint") {
      selectedValue = "Barba";
      console.log(`Replaced "SharePoint" with "Barba"`);
    } else if (selectedValue === "PowerApps") {
      selectedValue = "Palma";
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="ml-1 bg-white p-6 rounded-lg shadow-lg border-2 border-gray-300 lg:col-start-3">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <h3 className="text-sm font-medium text-gray-500">Extra Profit</h3>
